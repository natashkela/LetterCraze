...
/*
 * This shows the user available levels in Puzzle
 * if they are unlocked or locked
 * the number of stars out of 3 for each level that user gained
 * 
 * */

public class SelectLevelPuzzle extends JPanel {
	SplashScreen1 bla;//needed this for constructor just named it bla because of no reason
	/**
	 * Create the panel.
	 */
	public SelectLevelPuzzle(SplashScreen1 first) {
		...
		btnUnlocked.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				bla.setContentPane(new PuzzleLevel(bla));
				bla.revalidate();
			}
		});
