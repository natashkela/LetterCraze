...
/*
 * Lightening builder allows user to add a lightening level so far
 * when you press it it takes you to AddLevelLightening which allows user to see
 * how it looks so far 
 * */
public class LighteningBuilder extends JPanel {

	SplashScreen2 bla;
	/**
	 * Create the panel.
	 */
	public LighteningBuilder(SplashScreen2 first) {
		this.bla=first;
		setLayout(null);
		
		JLabel lblPuzzleLevelBuilder = new JLabel("Lightening Level Builder");
		...
		btnAddLevel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				bla.setContentPane(new AddLevelLightening(bla));
				bla.revalidate();
			}
		});
